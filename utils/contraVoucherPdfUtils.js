...
/**
 * Generates a professional 4-column boxy-format PDF for a Contra Voucher with Serial Number.
 */
export const generateContraVoucherPDF = async (data, filePath) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            const { voucherNo, date, total, transactions, narration } = data;
            const companyName = data.company?.name || data.companyName || "Cloudsat Private Limited";
            ...
            doc.fontSize(14).font("Helvetica-Bold").text("CONTRA VOUCHER", { align: "center", underline: true });
            doc.moveDown(1.5);
            ...
            doc.text("From Account (Credit)", 85, tableY + 5);
            doc.text("To Account (Debit)", 265, tableY + 5);
            ...
            doc.moveTo(260, contentStartY).lineTo(260, contentStartY + contentHeight).stroke();
            ...
            transactions.forEach((t, index) => {
                const amountStr = `Rs. ${parseFloat(t.amount).toLocaleString("en-IN", { minimumFractionDigits: 2 })}`;
                
                doc.text(`${index + 1}`, 45, currentY, { width: 30, align: 'center' });
                doc.text(t.fromAccountName || "N/A", 85, currentY, { width: 170 });
                doc.text(t.toAccountName || "N/A", 265, currentY, { width: 170 });
            ...
            // Global Narration (Narrative)
            doc.fontSize(10).font("Helvetica-Bold").text("Narration:", 40, footerY);
            doc.fontSize(10).font("Helvetica").text(narration || "Transfer between accounts", 110, footerY, { width: 420 });
            footerY = doc.y + 20;
            ...
            // Authorised Signatory at the end

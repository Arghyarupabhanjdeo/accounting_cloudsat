...
/**
 * Generates a professional boxy-format PDF for a Journal Voucher.
 */
export const generateJournalVoucherPDF = async (data, filePath) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            const { voucherNo, date, items, totalDebit, totalCredit, narration } = data;
            ...
            doc.fontSize(14).font("Helvetica-Bold").text("JOURNAL VOUCHER", { align: "center", underline: true });
            doc.moveDown(1.5);
            ...
            doc.text("Debit (Rs.)", 360, tableY + 5, { align: "right", width: 90 });
            doc.text("Credit (Rs.)", 460, tableY + 5, { align: "right", width: 90 });
            ...
            doc.moveTo(365, contentStartY).lineTo(365, contentStartY + contentHeight).stroke();
            doc.moveTo(465, contentStartY).lineTo(465, contentStartY + contentHeight).stroke();
            ...
            if (items && items.length > 0) {
                items.forEach((item, index) => {
                    const drStr = item.debit && parseFloat(item.debit) > 0 ? parseFloat(item.debit).toLocaleString("en-IN", { minimumFractionDigits: 2 }) : "";
                    const crStr = item.credit && parseFloat(item.credit) > 0 ? parseFloat(item.credit).toLocaleString("en-IN", { minimumFractionDigits: 2 }) : "";
                    
                    doc.text(`${index + 1}`, 45, currentY, { width: 30, align: 'center' });
                    doc.text(item.description || "N/A", 85, currentY, { width: 270 });
                    doc.text(drStr, 360, currentY, { align: "right", width: 85 });
                    doc.text(crStr, 460, currentY, { align: "right", width: 85 });
            ...
            /* ================= TOTAL DISPLAY ================= */
            const totalBoxY = contentStartY + contentHeight - 30;
            doc.font("Helvetica-Bold").fontSize(10);
            doc.text("Total:", 85, totalBoxY);
            doc.text(`${parseFloat(totalDebit).toLocaleString("en-IN", { minimumFractionDigits: 2 })}`, 360, totalBoxY, { align: "right", width: 85 });
            doc.text(`${parseFloat(totalCredit).toLocaleString("en-IN", { minimumFractionDigits: 2 })}`, 460, totalBoxY, { align: "right", width: 85 });
            ...
            // Amount in words (using Debit total)
            doc.fontSize(10).font("Helvetica-Bold").text("Amount (in words):", 40, footerY);
            doc.fontSize(10).font("Helvetica").text(`INR ${numberToWords(totalDebit)}`, 135, footerY, { width: 380 });
            
            // Authorised Signatory
            doc.fontSize(10).font("Helvetica").text("Authorised Signatory", 400, 750, { align: "center", width: 155 });
